...
class SettingsImportActivity : AppCompatActivity() {
    ...
    private val assignLauncher = registerForActivityResult(
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            val formatterDate = SimpleDateFormat("MMMM dd, yyyy", Locale.getDefault()).apply { timeZone = tz }
            val formatterTime = SimpleDateFormat("HH:mm:ss", Locale.getDefault()).apply { timeZone = tz }
            ...
            for ((sid, samples) in byNewSession) {
                ...
                val date = formatterDate.format(Date(firstMs))
                val timeStart = formatterTime.format(Date(firstMs))
                val timeEnd = formatterTime.format(Date(lastMs))
        ...
        CoroutineScope(Dispatchers.IO).launch {
                    ...
                    cursor.use { c ->
                        ...
                        while (c.moveToNext()) {
                            try {
                                ...
                                if (prev == null) sessionRanges[sid] = ts to ts else sessionRanges[sid] = min(prev.first, ts) to max(prev.second, ts)
                            } catch (_: Exception) {
                                skipped++
                            }
                        }
            ...
            if (swimDataList.isNotEmpty()) {
                withContext(Dispatchers.Main) {
                    ...
                    val intent = Intent(this@SettingsImportActivity, ImportAssignActivity::class.java).apply {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            if (swimDataList.isNotEmpty()) {
                withContext(Dispatchers.Main) {
                    ...
                    val intent = Intent(this@SettingsImportActivity, ImportAssignActivity::class.java)
                    assignLauncher.launch(intent)
                }
            } else {
                withContext(Dispatchers.Main) {
