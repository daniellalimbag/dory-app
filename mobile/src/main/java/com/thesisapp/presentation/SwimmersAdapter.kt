...
import com.thesisapp.data.Goal
import com.thesisapp.data.GoalProgress
import com.thesisapp.data.GoalType
...
class SwimmersAdapter(
            ...
            if (goal != null) {
                ...
                withContext(Dispatchers.Main) {
                    if (latestProgress != null) {
                        ...
                        val color = when {
                            difference < 0 -> ContextCompat.getColor(holder.itemView.context, R.color.accent) // Green - beating goal
                            difference <= 1.0 -> ContextCompat.getColor(holder.itemView.context, android.R.color.holo_orange_dark) // Orange - close
                            else -> ContextCompat.getColor(holder.itemView.context, R.color.error) // Red - far from goal
                        }
                        
                        holder.tvGoalProgress.setTextColor(color)
                        holder.goalProgressIndicator.backgroundTintList = android.content.res.ColorStateList.valueOf(color)
    ...
    private suspend fun generateDummyProgressData(db: AppDatabase, goalId: Int) {
        ...
        val progressPoints = listOf(
            ...
            GoalProgress(0, goalId, calendar.apply { add(Calendar.MONTH, 1) }.timeInMillis, "1:03.20", null),
            GoalProgress(0, goalId, calendar.apply { add(Calendar.MONTH, 1) }.timeInMillis, "1:01.80", null),
            GoalProgress(0, goalId, calendar.apply { add(Calendar.MONTH, 1) }.timeInMillis, "0:59.50", null)
        )
